...
async function initializeDatabase() {
  ...
  try {
    ...
    const requiredCollections = [
      ...
      'pending_whatsapp_checkouts' // Collection for WhatsApp checkout data
    ];
    ...
    await db.collection('pending_whatsapp_checkouts').createIndex({ customerPhone: 1, status: 1 }); // Index for checkout lookups
